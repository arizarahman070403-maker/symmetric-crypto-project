...
for file in files:
    ...
    results.append([os.path.basename(file), "RC4", enc_time, dec_time])
    ...
    results.append([os.path.basename(file), "AES", enc_time, dec_time])

df = pd.DataFrame(results, columns=["File", "Algorithm", "Encryption Time", "Decryption Time"])
df.to_csv("performance_results.csv", index=False)
print(df)

import matplotlib.pyplot as plt
pivot = df.pivot(index="File", columns="Algorithm", values="Encryption Time")
pivot.plot(kind="bar")
